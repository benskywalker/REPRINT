...
const Relationships = ({nodeData, handleNodeClick}) => {
  const [relationships, setRelationships] = useState([]);
    ...
    const [globalFilter, setGlobalFilter] = useState(''); 
      ...
      const downloadPDF = () => {
          const doc = new jsPDF();
          const tableColumn = ["Name", "Relationship", "Relationship2", "Started", "Ended", "End reason"];
          const tableRows = [];
          ...
          const table = new Table({
              rows: [
                  ...
                  ...getFilteredData().map(doc => new TableRow({
                      children:  [
                          new TableCell({ children: [new Paragraph(doc.person?.fullName || 'Person not found')] }),
                          new TableCell({ children: [new Paragraph(doc.relationship1to2Desc || 'Relationship not found')] }),
                          new TableCell({ children: [new Paragraph(doc.relationship2to1Desc || 'Relationship not found')] }),
                          new TableCell({ children: [new Paragraph(doc.dateStart || 'Date not found')] }),
                          new TableCell({ children: [new Paragraph(doc.dateEnd || 'Date not found')] }),
                          new TableCell({ children: [new Paragraph(doc.dateEndCause || 'Date not found')] }),
     ...
     useEffect(() => {
      ...
      const savedGlobalFilter = sessionStorage.getItem('relationships-table-globalFilter');
  ...
  const onFilter = (e) => {
      ...
      sessionStorage.setItem('relationships-table-filters', JSON.stringify(e.filters)); // Save to sessionStorage
  };
