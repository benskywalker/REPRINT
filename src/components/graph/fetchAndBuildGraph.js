...
const fetchAndBuildGraph = async (nodesUrl, edgesUrl, filters, edgeTypeFilters, body, selectedTerms, dateRange) => {
  ...
  try {
    ...
    // Apply filtering
    const graph = buildGraph(nodes, edges, filters, edgeTypeFilters, selectedTerms, dateRange)
...
const buildGraph = (nodes, edges, filters, edgeTypeFilters, selectedTerms, dateRange) => {
  ...
  const dateMatch = relevantDates.some(date => 
    ...
    const matchedNodeIds = new Set(filteredNodesMatchingTerms.map(node => node.id));
