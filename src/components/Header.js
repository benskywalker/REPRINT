...
const Header = ({ onUploadClick }) => {
  ...
  return (
    <div className={styles.header}>
      <NavLink to="/" activeclassname={styles.activeLink} className={styles.title}>Network Analysis</NavLink>
      {/* search bar in middle of header */}
      <InputText placeholder='Search' className={styles.search} />
      {/* pi bar menu for sidebar pop up */}
      <Button icon="pi pi-bars" className={styles.menu} onClick={toggleSidebar} />
      <Sidebar visible={sidebarVisible} position="right" onHide={() => setSidebarVisible(false)}>
        ...
        <ul className={styles.routesList}>
            ...
            <li><NavLink to="/query-tool" activeclassname={styles.activeLink}>Query Tool</NavLink></li>
            <li><NavLink to="/gallery" activeclassname={styles.activeLink}>Gallery</NavLink></li>
        </ul>
        <hr></hr>
      </Sidebar>
    </div>
